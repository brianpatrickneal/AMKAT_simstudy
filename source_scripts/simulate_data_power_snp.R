  ...
  foreach::foreach(
    ...
    .packages = packages_to_pass_to_foreach) %dopar% { # pass chunks to workers
      foreach(i = chunk, .combine = cbind) %do% { # worker iterates over chunk
        ...
        # AMKAT, No Filter
        observed_statistic <- AMKAT:::.generateTestStatNoFilter(
          null_residuals, null_standard_errors, x,
          candidate_kernels)$test_statistic
        permutation_statistics <- AMKAT:::.generatePermStatsNoFilter(
          null_residuals, null_standard_errors,
          x, candidate_kernels, num_permutations)
